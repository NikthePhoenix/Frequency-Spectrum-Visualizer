...
#define SAMPLING_FREQ   11590         // Hz, must be 40000 or less due to ADC conversion time. Determines maximum frequency that can be analysed by the FFT Fmax=sampleF/2.
...
#define NUM_BANDS       14            // To change this, you will need to change the bunch of if statements describing the mapping from bins to bands
...
void loop() {
  ...
  for (int i = 2; i < (SAMPLES/2); i++){       // Don't use sample 0 and only first SAMPLES/2 are usable. Each array element represents a frequency bin and its value the amplitude.
    ...
    /*16 bands, 12kHz top band
      ...
      //14 bands, 5.795kHz top band
      if (i<=2 )          bandValues[0]  += (int)vReal[i];
      if (i>2  && i<=3  ) bandValues[1]  += (int)vReal[i];
      if (i>3  && i<=4  ) bandValues[2]  += (int)vReal[i];
      if (i>4  && i<=6  ) bandValues[3]  += (int)vReal[i];
      if (i>6  && i<=8  ) bandValues[4]  += (int)vReal[i];
      if (i>8  && i<=11 ) bandValues[5]  += (int)vReal[i];
      if (i>11 && i<=15 ) bandValues[6]  += (int)vReal[i];
      if (i>15 && i<=21 ) bandValues[7]  += (int)vReal[i];
      if (i>21 && i<=29 ) bandValues[8]  += (int)vReal[i];
      if (i>29 && i<=41 ) bandValues[9]  += (int)vReal[i];
      if (i>41 && i<=57 ) bandValues[10] += (int)vReal[i];
      if (i>57 && i<=79 ) bandValues[11] += (int)vReal[i];
      if (i>79 && i<=110) bandValues[12] += (int)vReal[i];
      if (i>110         ) bandValues[13] += (int)vReal[i];
